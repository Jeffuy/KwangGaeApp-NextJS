...
const AlbumPage14 = ({ user, loading, cardList }) => {
	...
	return (
		<section className="album-page4-bg">
			<h2 className="album-page-title"> Panamericano 2016 (1/2)</h2>
			<div className={` ${expand != 0 ? 'album-page-expand' : 'album-page'}`}>
				{cardList?.map(sticker => (
					<React.Fragment key={sticker.number}>
						{sticker.number <= 121 && sticker.number >= 114 ? (
							<>
								{(sticker.number === 115 || sticker.number === 116 || sticker.number === 90 || sticker.number === 65) && <div className={` ${expand != 0 ? '' : 'album-break'}`} />}
