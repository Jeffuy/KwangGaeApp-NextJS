...
const AlbumPage12 = ({ user, loading, cardList }) => {
	...
	return (
		<section className="album-page4-bg">
			<h2 className="album-page-title"> Centro y Sudamericano 2019 (2/3)</h2>
			<div className={` ${expand != 0 ? 'album-page-expand' : 'album-page'}`}>
				{cardList?.map(sticker => (
					<React.Fragment key={sticker.number}>
						{sticker.number <= 106 && sticker.number >= 100 ? (
							<>
								{(sticker.number === 101 || sticker.number === 88 || sticker.number === 90 || sticker.number === 65) && <div className={` ${expand != 0 ? '' : 'album-break'}`} />}
