...
const AlbumPage6 = ({ user, loading, cardList }) => {
	...
	return (
		<section className="album-page4-bg">
			<h2 className="album-page-title"> Paso de los Toros 2022 </h2>
			<div className={` ${expand != 0 ? 'album-page-expand' : 'album-page'}`}>
				{cardList?.map(sticker => (
					<React.Fragment key={sticker.number}>
						{sticker.number <= 54 && sticker.number >= 45 ? (
							<>
								{(sticker.number === 47 || sticker.number === 49 || sticker.number === 34 || sticker.number === 50 || sticker.number === 52) && (
