...
const AlbumPage11 = ({ user, loading, cardList }) => {
	...
	return (
		<section className="album-page4-bg">
			<h2 className="album-page-title"> Centro y Sudamericano 2019 (1/3)</h2>
			<div className={` ${expand != 0 ? 'album-page-expand' : 'album-page'}`}>
				{cardList?.map(sticker => (
					<React.Fragment key={sticker.number}>
						{sticker.number <= 99 && sticker.number >= 92 ? (
							<>
								{(sticker.number === 86 || sticker.number === 88 || sticker.number === 90 || sticker.number === 65) && <div className={` ${expand != 0 ? '' : 'album-break'}`} />}
