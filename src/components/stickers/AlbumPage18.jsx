...
const AlbumPage18 = ({ user, loading, cardList }) => {
	...
	return (
		<section className="album-page4-bg">
			<h2 className="album-page-title"> World Cup 2022 (1/5)</h2>
			<div className={` ${expand != 0 ? 'album-page-expand' : 'album-page'}`}>
				{cardList?.map(sticker => (
					<React.Fragment key={sticker.number}>
						{sticker.number <= 161 && sticker.number >= 152 ? (
							<>
								{(sticker.number === 153 || sticker.number === 154 || sticker.number === 146 || sticker.number === 148 || sticker.number === 150) && (
