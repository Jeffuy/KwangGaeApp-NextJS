...
export const MarketContextProvider = ({ children }) => {
	...
	const secondQuiz = async () => {
		...
		if (user && userData && !userData.secondQuiz) {
			...
			setMessageQuiz('Logueate para ganar los puntos');
		} else if (userData.secondQuiz) {
			setMessageQuiz('Ya has reclamado los puntos para este quiz');
		}
		setClicked(false);
	};

	const thirdQuiz = async () => {
		setClicked(true);
		console.log('clicked');
		if (user && userData && !userData.thirdQuiz) {
			setDoc(doc(db, 'users', user.uid), { availablePoints: userData?.availablePoints + 100, thirdQuiz: true, points: userData?.points + 100 }, { merge: true });
			setMessageQuiz('Has ganado 100 puntos');
		} else if (!user || !userData) {
			setMessageQuiz('Logueate para ganar los puntos');
		} else if (userData.thirdQuiz) {
			setMessageQuiz('Ya has reclamado los puntos para este quiz');
		}
		setClicked(false);
	};

	const fourthQuiz = async () => {
		setClicked(true);
		console.log('clicked');
		if (user && userData && !userData.fourthQuiz) {
			setDoc(doc(db, 'users', user.uid), { availablePoints: userData?.availablePoints + 100, fourthQuiz: true, points: userData?.points + 100 }, { merge: true });
			setMessageQuiz('Has ganado 100 puntos');
		} else if (!user || !userData) {
			setMessageQuiz('Logueate para ganar los puntos');
		} else if (userData.thirdQuiz) {
